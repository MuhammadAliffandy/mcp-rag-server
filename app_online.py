...
if st.session_state.get("processing_pending", False):
    ...
    with st.spinner("Medical AI is planning and executing..."):
        ...
        if tool == "rag":
            ...
            TOOL_LABEL_MAP = {
                "query_core_rag": "Retrieving Patient Context",
                "query_guard_rag": "Consulting Medical Guidelines",
                "execute_pinebio_ml": "Executing ML Statistical Analysis"
            }
            ...
            for i, task in enumerate(tasks):
                ...
                if t_name == "query_core_rag":
                    with st.spinner("Retrieving Patient Context..."):
                        m_res = asyncio.run(call_mcp_tool("query_core_rag", t_args))
                        st.markdown(m_res)
                        res += f"\n- {m_res}"
                ...
                elif t_name == "execute_pinebio_ml":
                    with st.spinner("Executing ML Statistical Analysis..."):
                        m_res = asyncio.run(call_mcp_tool("execute_pinebio_ml", t_args))
                        st.markdown(m_res)
                        res += f"\n- {m_res}"
                        tool_outputs.append(f"PineBio ML Output: {m_res}")
